...
func (s *OperationService) MakeReportLink(ctx context.Context, month int, year int) (string, error) {
...
func (s *OperationService) MakeReportFile(ctx context.Context, month int, year int) ([]byte, error) {
	products, amounts, err := s.operationRepo.GerAllRevenueOperationsGroupedByProduct(ctx, month, year)
	if err != nil {
		return nil, errors.New("failed to get revenue operatuions")
	}
